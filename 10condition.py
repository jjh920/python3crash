...
# 조건이 참일때값 if 조건식 else 조건이 거짓일때 값
...
income = int(input('소득이 얼마일까요?'))
isTake = int(input('다른 지원금을 받고 있나요?(1. 받고있다. 2. 받고있지 않다.'))
isTarget = (income < 4000000) and (isTake == '2')

result = '대상자 입니다.' if isTarget else '비대상자 입니다.'

print(f'{income} {isTake} {result}')

# 모듈
# 타인이 만든 특정 기능을 모아둔 파일 (재사용 목적)
...
money = int(input('무엇으로 구매했습니까? 1.달러 2.유로'))

doller = 1298.80
euro = 1428.94

dlrnb = op.mul(780, doller)
errnb = op.mul(650, euro)

result = '달러가 더 비쌈' if dlrnb > errnb \
        else '유로가 더 비쌈'
