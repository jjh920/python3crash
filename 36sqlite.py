...
# .mode tabs
# .import tsv 파일명 테이블명
# .import c:/Java/sqlite/zipcode_20130201(2).txt zipcode
...
# 실행할 SQL문을 작성하고
# 실행후 결과를 커서에 저장
sql = 'select * from zipcode where dong like "중앙%" and gugun like "관악구"'
cursor.execute(sql)

# 반복문을 이용해서 커서에 저장된
# 결과집합으로 부터 한행씩 읽어서 처리
while True:
    row = cursor.fetchone()
    if not row: break
...
sql = 'select * from employees where job_id like "it%"'
cursor.execute(sql)
...
sql = (' select FIRST_NAME, LAST_NAME, job_id, salary, d.depatment_name '
       ' from employees inner join departments d '
       ' using (department_id) limit 0, 20 ')
cursor.execute(sql)
